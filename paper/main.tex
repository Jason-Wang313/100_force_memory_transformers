...
\usepackage{url}
\usepackage{booktabs}
\title{Force Memory Transformers: ICLR Main-Conference Gate Archive}
\author{Anonymous Authors}
\begin{document}
\maketitle
\begin{abstract}
This document is not an ICLR main-conference submission. It is the v3 main-conference gate for a generated robotics draft about transformer robot policies. The original research bet was: Give transformers persistent force-memory channels that survive visual scene changes. After a stricter hostile review, the honest terminal decision is \textbf{KILL/ARCHIVE}. Submission-hardening version: v3.
\end{abstract}
...
\begin{itemize}
\item ZeroRel: Multimodal Transformer-Guided Zero-Shot Relationship Retrieval for Generalized Scene Graph Generation (2026)
\item Scene-LLM: Extending Language Model for 3D Visual Reasoning (2025)
\item Semantic-geometric visual place recognition: a new perspective for reconciling opposing views (2022)
\item Surgical-VQLA:Transformer with Gated Vision-Language Embedding for Visual Question Localized-Answering in Robotic Surgery (2023)
\item Evaluating Multimodal Vision-Language Model Prompting Strategies for Visual Question Answering in Road Scene Understanding (2025)
\end{itemize}
